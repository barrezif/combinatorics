...
class VennGraph:
    ...
    def get_sets(self, *names: VennSetName)->Iterator[VennSet]:
        """ An Iterator for sets that include the given set
            ex: sets("A") -> ("A", "AB", "AC", "ABC")
        """
        return self._get_set_from_names(names).get_child_sets()

    def get_all_set_names(self)->Iterator[VennSetName]:
        """ An Iterator for sets that include the given set
            ex: sets("A") -> ("A", "AB", "AC", "ABC")
        """
        print("get_all_set_names", self.sets.keys())
        return [name for name in self.sets.keys()]
